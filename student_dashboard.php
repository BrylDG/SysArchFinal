    ...
    <div class="container mx-auto p-6">
        <div class="card rounded-xl shadow-lg border border-secondary p-6 hover:shadow-xl transition-all duration-300">
            ...
            <div class="flex flex-col md:flex-row gap-6 items-start mb-8">
                <!-- Profile Picture with Sessions Badge -->
                <div class="relative">
                    <img 
                        src="uploads/<?php echo htmlspecialchars($profile_picture); ?>" 
                        alt="Profile Picture" 
                        class="w-32 h-32 rounded-full border-4 border-primary object-cover shadow-md"
                        onerror="this.src='assets/default_avatar.png'"
                    >
                    <!-- Sessions Badge -->
                    <div class="absolute -bottom-2 -right-2 bg-primary text-white rounded-full px-4 py-1 shadow-lg flex items-center">
                        <span class="text-sm font-bold"><?php echo htmlspecialchars($remaining_sessions); ?></span>
                        <span class="ml-1 text-xs">sessions</span>
                    </div>
                </div>
                ...
                <div class="flex-1">
                    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        <div>
                            <h2 class="text-2xl font-bold text-dark"><?php echo htmlspecialchars($firstname . " " . $lastname); ?></h2>
                            <p class="text-primary font-medium">@<?php echo htmlspecialchars($idno); ?></p>
                        </div>
                        
                        <!-- Quick Actions -->
                        <div class="flex gap-2">
                            <a href="edit-profile.php" class="px-3 py-1 bg-secondary/50 hover:bg-secondary rounded-full text-sm font-medium transition-all">
                                <i class="fas fa-user-edit mr-1"></i> Edit
                            </a>
                            <a href="reservation.php" class="px-3 py-1 bg-primary hover:bg-primary/90 text-white rounded-full text-sm font-medium transition-all">
                    ...
                    <div class="mt-4 grid grid-cols-2 md:grid-cols-3 gap-3">
                        ...
                        <div class="bg-secondary/20 p-3 rounded-lg">
                            ...
                            <p class="font-medium truncate"><?php echo htmlspecialchars($email); ?></p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <!-- Quick Links Section -->
            <div class="mt-8">
                <h3 class="text-xl font-semibold mb-4 text-dark">Quick Links</h3>
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <a href="reservation.php" class="card p-4 rounded-lg hover-effect transition-all duration-200 text-center">
                        <i class="fas fa-calendar-check text-primary text-2xl mb-2"></i>
                        <p class="font-medium">Reservation</p>
                    </a>
                    <a href="sit_in_history.php" class="card p-4 rounded-lg hover-effect transition-all duration-200 text-center">
                        <i class="fas fa-history text-primary text-2xl mb-2"></i>
                        <p class="font-medium">Sit-in History</p>
                    </a>
                    <a href="sit-in-rules.php" class="card p-4 rounded-lg hover-effect transition-all duration-200 text-center">
                        <i class="fas fa-book text-primary text-2xl mb-2"></i>
                        <p class="font-medium">Sit-in Rules</p>
                    </a>
                    <a href="lab-rules.php" class="card p-4 rounded-lg hover-effect transition-all duration-200 text-center">
                        <i class="fas fa-clipboard-list text-primary text-2xl mb-2"></i>
                        <p class="font-medium">Lab Rules</p>
                    </a>
                    <a href="student_leaderboard.php" class="card p-4 rounded-lg hover-effect transition-all duration-200 text-center">
                        <i class="fas fa-trophy text-primary text-2xl mb-2"></i>
                        <p class="font-medium">Leaderboard</p>
                    </a>
                    <a href="upload_resources.php" class="card p-4 rounded-lg hover-effect transition-all duration-200 text-center">
                        <i class="fas fa-file-alt text-primary text-2xl mb-2"></i>
                        <p class="font-medium">Resources</p>
                    </a>
                    <a href="student_lab_schedule.php" class="card p-4 rounded-lg hover-effect transition-all duration-200 text-center">
                        <i class="fas fa-clock text-primary text-2xl mb-2"></i>
                        <p class="font-medium">Lab Schedule</p>
                    </a>
                    <a href="announcements.php" class="card p-4 rounded-lg hover-effect transition-all duration-200 text-center">
                        <i class="fas fa-bullhorn text-primary text-2xl mb-2"></i>
            ...
            <div class="mt-8">
                <div class="flex justify-between items-center mb-4">
                    <h3 class="text-xl font-semibold text-dark">Latest Announcements</h3>
                    <a href="announcements.php" class="text-sm text-primary hover:text-primary/80">View All</a>
                </div>
                
                <div class="space-y-4">
                    <?php
                    // Fetch latest 3 announcements
                    $announcement_query = "SELECT title, message, created_at FROM announcements ORDER BY created_at DESC LIMIT 3";
                    ...
                    if (mysqli_num_rows($announcement_result) > 0) {
                        while($announcement = mysqli_fetch_assoc($announcement_result)) {
                            echo '<div class="announcement-card p-6 rounded-lg border border-secondary hover-effect transition-all duration-200">';
                            echo '<h4 class="text-lg font-semibold mb-2 text-dark">' . htmlspecialchars($announcement['title']) . '</h4>';
                            echo '<p class="text-sm text-dark mb-4">' . nl2br(htmlspecialchars($announcement['message'])) . '</p>';
                            echo '<p class="text-xs text-secondary"><small>Posted on: ' . htmlspecialchars($announcement['created_at']) . '</small></p>';
                            echo '</div>';
                        }
                    } else {
                        echo '<div class="card p-4 rounded-lg text-center text-secondary">No announcements available</div>';
                    }
                    ?>
                </div>
            </div>
